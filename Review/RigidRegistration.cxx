...
// global ini file that is saved after each iteration to interface with avs

const int MAXLEN = 512;
char meshXformCmd[MAXLEN], TransformMeshCmd[MAXLEN]; // cmds for mesh transform

typedef itk::VersorRigid3DTransform< double > TransformType;
TransformType::Pointer  transform = TransformType::New();

      
int WriteIni(double , int , TransformType::MatrixType Matrix,
  ...
  void Execute(const itk::Object * object, const itk::EventObject & event)
    {
      ...
      std::cout << "Offset = " << offset 
                               << " Dimensions in **MilliMeters**" << std::endl;
      std::cout << "Matrix = " << std::endl << matrix << std::endl;

      WriteIni( optimizer->GetValue(),optimizer->GetCurrentIteration(),
                                                                matrix,offset);
    }
};
...
int main( int argc, char *argv[] )
{
  if( argc < 4 )
    ...
    std::cerr << " fixedImageFile movingImageFile controlFile     "<< std::endl;
    std::cerr << " data from preop_dicom_dir used as FixedImage   "<< std::endl;
    std::cerr << " data from interop_dicom_dir used as MovingImage"<< std::endl;
    std::cerr << " meshXformCmd used to apply mesh transformations"<< std::endl;
    return 1;
    }
  ...
  std::cout << "Ini File Written in **Meters** " << std::endl ;

  CSimpleIni  ControlIni;  // instantiate ini control file
  bool bHasMulti;  // flag for multiple ini keywords
  // open current control file
  SI_Error rc = ControlIni.LoadFile(argv[3]);

  if (rc < 0) { printf("Failed to open %s\n",argv[3]); abort(); }
  // too slow to execute the mesh conversion so only echo it...
  sprintf(meshXformCmd,"echo %s",
     ControlIni.GetValue("registration","meshxformcmd","",&bHasMulti));
  // echo parameters
  std::cout << "meshXformCmd    "<< meshXformCmd    << std::endl ;
  ...
  writer2->SetFileName( "afterReg.mha" );
  writer2->Update();
  ...
  writer2->SetFileName( "beforeReg.mha" );
  writer2->Update();
...
int WriteIni(double Value, int IterNo, TransformType::MatrixType Matrix,
                                     TransformType::OffsetType Offset){

  
  CSimpleIni  RegIni;  // instantiate ini file to hold registration parameters
  char FileName[MAXLEN],tmpVal[64];


  sprintf(tmpVal,"%e",Value);
  RegIni.SetValue( "registration_params", "value" , tmpVal  );
  sprintf(tmpVal,"%f",Matrix(0,0));
  RegIni.SetValue( "registration_params", "RotMat00" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(0,1));
  RegIni.SetValue( "registration_params", "RotMat01" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(0,2));
  RegIni.SetValue( "registration_params", "RotMat02" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(1,0));
  RegIni.SetValue( "registration_params", "RotMat10" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(1,1));
  RegIni.SetValue( "registration_params", "RotMat11" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(1,2));
  RegIni.SetValue( "registration_params", "RotMat12" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(2,0));
  RegIni.SetValue( "registration_params", "RotMat20" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(2,1));
  RegIni.SetValue( "registration_params", "RotMat21" ,tmpVal);
  sprintf(tmpVal,"%f",Matrix(2,2));
  RegIni.SetValue( "registration_params", "RotMat22" ,tmpVal);
  // write offset in meters
  sprintf(tmpVal,"%f",0.001*Offset[0]);
  RegIni.SetValue( "registration_params", "OffSet0" ,tmpVal);
  sprintf(tmpVal,"%f",0.001*Offset[1]);
  RegIni.SetValue( "registration_params", "OffSet1" ,tmpVal);
  sprintf(tmpVal,"%f",0.001*Offset[2]);
  RegIni.SetValue( "registration_params", "OffSet2" ,tmpVal);

  sprintf(FileName,"registration%04d.ini", IterNo);
  RegIni.SaveFile(FileName);

  
  sprintf(TransformMeshCmd,meshXformCmd,FileName);
  system(TransformMeshCmd);
