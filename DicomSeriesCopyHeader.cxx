...
int main( int argc, char* argv[] )
{
  if( argc < 4 )
    ...
    std::cerr << "Usage: " << argv[0] << 
      " [Header Data Dir] [Pixel Data Dir] [Output Dicom Directory]" << std::endl;
    return EXIT_FAILURE;
    }
...
// Software Guide : BeginCodeSnippet
  ...
  ImageIOType::Pointer origingdcmIO = ImageIOType::New();
  ImageIOType::Pointer modifygdcmIO = ImageIOType::New();
  NamesGeneratorType::Pointer originnamesGenerator = NamesGeneratorType::New();
  NamesGeneratorType::Pointer modifynamesGenerator = NamesGeneratorType::New();
// Software Guide : EndCodeSnippet
...
// Software Guide : BeginCodeSnippet
  originnamesGenerator->SetInputDirectory( argv[1] );
  modifynamesGenerator->SetInputDirectory( argv[2] );

  const ReaderType::FileNamesContainer & originfilenames = 
                            originnamesGenerator->GetInputFileNames();
  const ReaderType::FileNamesContainer & modifyfilenames = 
                            modifynamesGenerator->GetInputFileNames();
// Software Guide : EndCodeSnippet
  ...
  std::cout << "origin # of file names "<< originfilenames.size() << std::endl; 
  for(unsigned int fni = 0; fni < originfilenames.size(); fni++)
    {
    std::cout << "filename # " << fni << " = ";
    std::cout << originfilenames[fni] << std::endl;
    }
  std::cout << "modify # of file names "<< modifyfilenames.size() << std::endl; 
  for(unsigned int fni = 0; fni < modifyfilenames.size(); fni++)
    {
    std::cout << "filename # " << fni << " = ";
    std::cout << originfilenames[fni] << std::endl;
    }
...
// Software Guide : BeginCodeSnippet
  ...
  ReaderType::Pointer modifyreader = ReaderType::New();

  originreader->SetImageIO(   origingdcmIO );
  originreader->SetFileNames( originfilenames );
  modifyreader->SetImageIO(   modifygdcmIO );
  modifyreader->SetFileNames( modifyfilenames );
// Software Guide : EndCodeSnippet
...
// Software Guide : EndCodeSnippet
  ...
  SliceIteratorType originIt( originreader->GetOutput(), 
                              originreader->GetOutput()->GetRequestedRegion() );
  SliceIteratorType modifyIt( modifyreader->GetOutput(), 
                              modifyreader->GetOutput()->GetRequestedRegion() );
...
/*         ________
 begin    |->|->|->|
   ...
   y       --------              direction of iteration of ITK
    ^     |->|->|->|             slice iterator w/
    |      --------              originIt.SetFirstDirection(  0 );
    |     |->|->|->|             originIt.SetSecondDirection( 1 );
    |      --------
    |     |->|->|->|   end 
          |--------|              ----------> x
*/
  originIt.SetFirstDirection(  0 );
  originIt.SetSecondDirection( 1 );
  modifyIt.SetFirstDirection(  0 );
  modifyIt.SetSecondDirection( 1 );
...
// Software Guide : BeginCodeSnippet
  ...
  while( !originIt.IsAtEnd() )
    { 
    while ( !originIt.IsAtEndOfSlice() )
      { 
...
// Software Guide : BeginCodeSnippet
  ...
  seriesWriter->SetInput( originreader->GetOutput() );
  seriesWriter->SetImageIO( origingdcmIO );
// Software Guide : EndCodeSnippet
...
// Software Guide : BeginCodeSnippet
  originnamesGenerator->SetOutputDirectory( outputDirectory );

  seriesWriter->SetFileNames( originnamesGenerator->GetOutputFileNames() );
// Software Guide : EndCodeSnippet
...
// Software Guide : BeginCodeSnippet
  seriesWriter->SetMetaDataDictionaryArray( 
                        originreader->GetMetaDataDictionaryArray() );
// Software Guide : EndCodeSnippet
